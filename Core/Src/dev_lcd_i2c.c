...
/* ĐỊNH NGHĨA CÁC LỆNH ĐIỀU KHIỂN LCD (HD44780 COMMANDS) */
/* Lệnh cơ bản */
...
/* Lệnh thiết lập chế độ nhập (Entry Mode) */
...
/* Lệnh thiết lập chức năng (Function Set) */
...
/* Định nghĩa địa chỉ bộ nhớ (DDRAM Address) */
...
static void LCD_Write_I2C(uint8_t data) {
	/* GỬI 1 BYTE XUỐNG MODULE PCF8574 QUA I2C1 */
	I2C1_Start();

	/* Bắt buộc dịch trái 1 bit để chừa chỗ cho bit Write (0) của chuẩn I2C */
	I2C1_WriteAddress(LCD_I2C_W_ADDR);
	I2C1_WriteData(data);
...
static void LCD_Send(uint8_t data, uint8_t mode) {
	/* CHIA DỮ LIỆU 8-BIT THÀNH 2 LẦN GỬI 4-BIT (HIGH NIBBLE & LOW NIBBLE) */
	...
	LCD_Write_I2C(high_nibble | LCD_EN);
	Delay_ms(1);
	LCD_Write_I2C(high_nibble & ~LCD_EN);
	Delay_ms(1);

	/* GỬI 4-BIT THẤP KÈM THEO XUNG CHÂN EN */
	LCD_Write_I2C(low_nibble | LCD_EN);
	Delay_ms(1);
	LCD_Write_I2C(low_nibble & ~LCD_EN);
	Delay_ms(1);
}
...
static void LCD_Send_Command(uint8_t cmd) {
	/* GỬI MỘT MÃ LỆNH ĐIỀU KHIỂN (RS = 0) */
	LCD_Send(cmd, 0);
}

static void LCD_Send_Data(uint8_t data) {
	/* GỬI MỘT KÝ TỰ ĐỂ HIỂN THỊ (RS = 1) */
	LCD_Send(data, LCD_RS);
}

static void LCD_SetCursor(uint8_t row, uint8_t col) {
	/* ĐỊNH VỊ CON TRỎ THEO TỌA ĐỘ HÀNG VÀ CỘT */
	uint8_t address;
	if (row == 0) {
...
static void LCD_PrintString(char *str) {
	/* IN TỪNG KÝ TỰ CỦA CHUỖI ĐẾN KHI KẾT THÚC (\0) */
	while (*str) {
...
void LCD_Init(void) {
	/* BƯỚC KHỞI TẠO BẮT BUỘC THEO DATASHEET CỦA CHIP HD44780 */
	Delay_ms(50);

	/* ÉP MÀN HÌNH VỀ CHẾ ĐỘ 4-BIT (GỬI LỆNH TRỰC TIẾP, KHÔNG DÙNG LCD_SEND) */
	LCD_Write_I2C(LCD_CMD_INIT_8BIT | LCD_EN); Delay_ms(5); LCD_Write_I2C(LCD_CMD_INIT_8BIT & ~LCD_EN); Delay_ms(5);
	LCD_Write_I2C(LCD_CMD_INIT_8BIT | LCD_EN); Delay_ms(1); LCD_Write_I2C(LCD_CMD_INIT_8BIT & ~LCD_EN); Delay_ms(5);
	LCD_Write_I2C(LCD_CMD_INIT_8BIT | LCD_EN); Delay_ms(1); LCD_Write_I2C(LCD_CMD_INIT_8BIT & ~LCD_EN); Delay_ms(5);
	LCD_Write_I2C(LCD_CMD_INIT_4BIT | LCD_EN); Delay_ms(1); LCD_Write_I2C(LCD_CMD_INIT_4BIT & ~LCD_EN); Delay_ms(5);

	/* THIẾT LẬP THÔNG SỐ HOẠT ĐỘNG */
	LCD_Send_Command(LCD_CMD_4BIT_2LINE);     /* Chế độ 4-bit, 2 dòng, font 5x8 */
	Delay_ms(1);
	LCD_Send_Command(LCD_CMD_DISPLAY_ON);     /* Bật màn hình, tắt con trỏ, không nháy */
	Delay_ms(1);
	LCD_Send_Command(LCD_CMD_CLEAR_DISPLAY);  /* Xóa sạch màn hình */
	Delay_ms(2);
	LCD_Send_Command(LCD_CMD_ENTRY_MODE_INC); /* Tự động dịch con trỏ sang phải khi viết */
	Delay_ms(1);
}
...
void LCD_Display_Status(uint8_t state, float temperature) {
	...
	switch (state) {
		case SYS_STATE_SAFE:
			LCD_PrintString("STATUS: SAFE");
			break;
		case SYS_STATE_WARNING:
			LCD_PrintString("STATUS: WARNING!");
			break;
		case SYS_STATE_DANGER:
			LCD_PrintString("STATUS: DANGER!!");
			break;
		case SYS_STATE_ERROR:
			LCD_PrintString("STATUS: ERROR");
			break;
		default:
			LCD_PrintString("STATUS: UNKNOWN");
			break;
	}
	...
	sprintf(temp_str, "Temp: %d.%d C", temp_int, temp_frac);

	LCD_PrintString(temp_str);
}
